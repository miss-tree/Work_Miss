          ...
          <div class="form_row">
            ...
            <van-popup v-model="showTicket" position="bottom">
              <van-picker
                ...
                title="发票类型"
                :columns="ticketType"
                @cancel="showTicket = false"
                @confirm="getTicketType"
              />
            </van-popup>
          </div>
          <div>
          ...
          <div class="form_row">
            ...
            <van-popup v-model="showWhy" position="bottom">
              <van-picker
                ...
                @cancel="showWhy = false"
                @confirm="getReturnWhy"
              />
            </van-popup>
          </div>
        </div>
        ...
        <div>
          ...
          <div v-show="ISRETURN=='是'">
            ...
            <div v-show="curIndex==1">
              <addreturn :RETURNORDER="RETURNORDER" :ORDER="ORDER" :proList="proList"></addreturn>
            </div>
            <!--退货结束-->
          </div>
        </div>
...
export default {
  ...
  methods: {
    getPart(value, index) {//获取选择的部门
      (this.SELECTPART = value), (this.showPart = false);
    },
    getTicketType(value, index) {//获取选择的发票类型
      (this.TICKETTYPE = value), (this.showTicket = false);
    },
    getReturnType(value, index) {//获取选择的退票类型
      (this.RETURNTICKET = value), (this.showReturn = false);
    },
    getReturnWhy(value, index) {//获取选择的退货原因
      (this.RETURNWHY = value), (this.showWhy = false);
    },
    applyAjax() {//请求客户数据
