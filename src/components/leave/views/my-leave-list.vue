		...
		<div class="dateRow" v-for="(item,index) in MOVELIST" @click="routerTo(item,index)">
			<div class="dateRowGrow">
				<div class="dateRowRight">
					...
					<div class="dateRow_right" :class="{span_green:item.state=='病假',span_red:item.state=='事假'}" >{{item.state}}</div>
				</div>
				<div class="dateRowRight">
					<div class="huise dateRow_left">{{item.part1}}</div>
					<div class="dateRow_right ">{{item.job}}</div>
				</div>
				<div class="dateRowRight">
					<div class="huise dateRow_left">{{item.days}}天</div>
	...
	export default {
		...
		methods: {
			routerTo(item,index){//页面跳转
					this.$router.push({path:"/leaveDetail",query:{id:item.id}})
			},
			setInfor(){
				setTimeout(()=>{
