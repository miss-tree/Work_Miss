<template>
	<div>
		...
		<div class="str-auto-complete-container" v-if="isShow">
			<div v-for="(item, index) in showList" :key="index" class="str-auto-complete-item" @tap="selectThisItem(item)" v-html="item.showString"></div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		filterList(stringExp) {
			...
			for (let i = 0; i < this.srcDataList.length; i++) {
				...
				var matches = this.matcher({ term: stringExp }, { text: temp });
				// console.log(matches);
				if (matches !== null) {
					tempArray.push({
						orginalString: temp,
